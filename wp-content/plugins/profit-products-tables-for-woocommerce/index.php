...
class WOOT {
    ...
    public function draw_table_data($args, $table_html_id, $as_script = 'woot-table-json-data') {
        ...
        $js_script_data = [
            ...
            'request_data' => [
                ...
                'woot_link_get_data' => json_encode($_GET)
            ]
        ];
     ...
     * @param array $request Table requested arguements
     * @return array or string
     */
    public function get_table_data($return = false, $request = []) {
        $posts = [];
        ...
        $args = array(
            ...
            'paged' => intval($request['current_page']) + 1
        );
        ...
        $shortcode_args_set = [];
        if (isset($request['shortcode_args_set'])) {
            $shortcode_args_set = unserialize((stripslashes($request['shortcode_args_set'])));

            //get all products, be care with memory overflow
        ...
        if (!isset($request['wp_columns_actions'])) {
            $request['wp_columns_actions'] = WOOT_WooCommerce::$action;
        }

        $profile = apply_filters($request['wp_columns_actions'], $table_id, $shortcode_args_set);

        if (!empty($profile)) {
            if (isset($profile[$request['orderby']])) {
                $orderby_field = $profile[$request['orderby']];
        ...
        if (!is_array($filter_data)) {
            ...
            $filter_data = json_decode(html_entity_decode(stripslashes($request['filter_data'])), ARRAY_N);
        }

        $filter_provider = trim($request['filter_provider']);
        ...
        //***
        $limit_found_posts = -1;
        if (isset($request['predefinition'])) {
            $predefinition = unserialize((stripslashes($request['predefinition'])));
        ...
        if (isset($request['lang'])) {
            $args['lang'] = apply_filters('woot_current_lang', $request['lang']);
        } else {
    ...
    public function get_smth() {
        ...
        switch ($what) {
            ...
            case 'shortcodes_set':
                $res = do_shortcode($_REQUEST['shortcodes_set']);
                break;
            ...
            default:
                $what = json_decode(stripslashes($_REQUEST['what']), true);
...
add_action('wp_loaded', function() {
    ...
    if (isset($_GET['woot-remote-page']) AND isset($_GET['anchor'])) {
        ?>
        var woot_remote_src = "<?php echo home_url($_GET['woot-remote-page']) ?>";
        var woot_remote_width = '<?php echo isset($_GET['width']) ? urldecode($_GET['width']) : '100%' ?>';
        var woot_remote_height = <?php echo isset($_GET['height']) ? intval($_GET['height']) : 500 ?>;
        var woot_remote_anchor = "#<?php echo strip_tags(strval($_GET['anchor'])) ?>";
        <?php
        echo 'var woot_link_get_data=' . json_encode($_GET) . ';';
