...
<?php if (boolval($published)): ?>
    ...
    <div class='data-table-23 woot-data-table <?php if (isset($table_view) AND $table_view) echo 'data-table-23-' . $table_view; ?> <?php echo $action ?> <?php echo $classes ?>' data-skin="<?php echo $skin ?>" id='<?php echo $table_html_id ?>'>
        <input type="search" data-key="<?php echo $search_data_key ?>" value="" minlength="<?php echo $text_search_min_symbols ?>" class="woot-text-search" <?php if ($hide_text_search): ?>style="display: none;"<?php endif; ?> placeholder="<?php echo $placeholder ?>" />
    ...
    <?php echo(isset($style) ? "<style data-table23-skin='{$skin}' data-table23-skin-of='{$table_html_id}'>" . $style . '</style>' : '') ?>
<?php else: ?>
