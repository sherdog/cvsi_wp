		...
		if (!empty($args['description'])) {
			...
			if (!empty($args['price'])) {
				...
				$parts = explode('.', trx_addons_parse_codes($args['price']));
					...
					if (count($parts) > 1 ) {
						if ($parts[0]!='') {
							?><span class="sc_price_currency"><?php echo wp_kses_post($parts[0]); ?></span><?php
						}						
						if ($parts[1]!='') {
							?><span class="sc_price_value"><?php echo wp_kses_post($parts[1]); ?></span><?php
						}							
						if ($parts[2]!='') {
							?><span class="sc_price_decimals"><?php echo wp_kses_post($parts[2]); ?></span><?php
						}
					} else {
						if (count($parts) == 1 ){
