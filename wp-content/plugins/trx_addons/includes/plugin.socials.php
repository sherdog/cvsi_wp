			...
			foreach ($list as $social) {
				...
				$link = str_replace(
					...
					array(
						...
						$email ? strip_tags($args['post_title']) : urlencode(strip_tags($args['post_title'])),
						$email ? strip_tags($args['post_descr']) : urlencode(strip_tags($args['post_descr'])),
						urlencode($args['post_thumb'])
					),
					is_array($url) ? $url['link'] : $url);
				$output .= '<a href="'.esc_url($link).'"'
					. ' class="social_item'.(!empty($popup) ? ' social_item_popup' : '').'"'
					. ($popup ? ' data-link="' . esc_url($link) .'"' : ' target="_blank"')
