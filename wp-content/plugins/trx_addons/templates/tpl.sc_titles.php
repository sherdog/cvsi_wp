<?php
/**
 * The template to display shortcode's title, subtitle info and description
 *
 * @package WordPress
...
if (!empty($args['counter'])) {
	?><span class="<?php 
			echo esc_attr(apply_filters('trx_addons_filter_sc_item_counter_class', 'sc_item_counter '.$sc.'_counter'.$align.$style, $sc));
			?>"><?php 
			trx_addons_show_layout(trx_addons_prepare_macros($args['counter']));
	?></span><?php
}
