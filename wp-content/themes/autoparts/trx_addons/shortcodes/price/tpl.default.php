...
?>><?php
	...
	if (!empty($args['image'])) {
		...
		?><img src="<?php echo esc_url($args['image']); ?>" alt="<?php echo esc_attr__('image', 'autoparts')?>" <?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
		if (!empty($args['link'])) {
			?></a><?php
		} else {
			?></div><?php
	...
	?><div class="sc_price_info"><?php
		...
		if (!empty($args['description'])) {
			?><div class="sc_price_description"><?php echo wp_kses(trx_addons_parse_codes($args['description']) ,'autoparts_kses_content'); ?></div><?php
		}
			...
			if (!empty($args['price'])) {
				...
				$parts = explode('.', trx_addons_parse_codes($args['price']));
					...
					if (count($parts) > 1 ) {
						if ($parts[0]!='') {
							?><span class="sc_price_currency"><?php echo wp_kses($parts[0] ,'autoparts_kses_content'); ?></span><?php
						}						
						if ($parts[1]!='') {
							?><span class="sc_price_value"><?php echo wp_kses($parts[1] ,'autoparts_kses_content'); ?></span><?php
						}							
						if ($parts[2]!='') {
							?><span class="sc_price_decimals"><?php echo wp_kses($parts[2] ,'autoparts_kses_content'); ?></span><?php
						}
					} else {
						if (count($parts) == 1 ){
								?><span class="sc_price_value"><?php echo wp_kses($parts[0] ,'autoparts_kses_content'); ?></span><?php					
						}
					}				
				?></div><?php			
			}		
