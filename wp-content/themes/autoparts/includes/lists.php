	...
	function autoparts_get_list_posts_types($prepend_inherit=false) {
		if (($list = autoparts_storage_get('list_posts_types'))=='') {
			$list = apply_filters('autoparts_filter_list_posts_types', array(
				'post' => esc_html__('Post', 'autoparts')
			));
...
//----------------------------------------------------
if ( ! function_exists( 'autoparts_get_list_sc_color_styles' ) ) {
    function autoparts_get_list_sc_color_styles( $prepend_inherit = false ) {
        $list = apply_filters(
            'autoparts_filter_get_list_sc_color_styles', array(
                'default' => esc_html__( 'Default', 'autoparts' ),
                'link2'   => esc_html__( 'Accent 2', 'autoparts' ),
                'link3'   => esc_html__( 'Accent 3', 'autoparts' ),
                'dark'    => esc_html__( 'Dark', 'autoparts' ),
            )
        );
        return $prepend_inherit ? autoparts_array_merge( array( 'inherit' => esc_html__( 'Inherit', 'autoparts' ) ), $list ) : $list;
    }
