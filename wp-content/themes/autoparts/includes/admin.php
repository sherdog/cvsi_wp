...
if ( !function_exists( 'autoparts_admin_scripts' ) ) {
	...
	function autoparts_admin_scripts() {
		...
		$screen = function_exists('get_current_screen') ? get_current_screen() : false;
		if (is_object($screen)) {
			if (autoparts_allow_override_options(!empty($screen->post_type) ? $screen->post_type : $screen->id)) {
				// Load fontello icons
				...
				wp_enqueue_style(  'fontello-animation', autoparts_get_file_url('css/fontello/css/animation.css') );
				// Load theme fonts
