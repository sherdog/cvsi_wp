			...
			if ($has_thumb 
				...
				// Put video under the thumb
					if ($post_format == 'video') {
						$video = autoparts_get_post_video($post_content, false);
					if (empty($video))
							$video = autoparts_get_post_iframe($post_content, false);
						if (empty($video)) {
							...
							if ( false === strpos( $post_content, 'wp-playlist-script' ) ) {
								...
								if (!empty($videos) && is_array($videos)) {
									$video = autoparts_array_get_first($videos, false);
								}
							}
						}
					if (!empty($video)) {
				...
				// Put audio over the thumb
						...
						$audio = autoparts_get_post_audio($post_content, false);
					if (empty($audio))
							$audio = autoparts_get_post_iframe($post_content, false);
						if (empty($audio)) {
							// Only get video from the content if a playlist isn't present.
							$post_content = apply_filters( 'the_content', get_the_content() );
							if ( false === strpos( $post_content, 'wp-playlist-script' ) ) {
								...
								if (!empty($audios) && is_array($audios)) {
									$audio = autoparts_array_get_first($audios, false);
							}
						}
							}
						if (!empty($audio)) {
	...
	function autoparts_single_comments_field($args) {
        ...
        if ( 'text' == $args['field_type'] ) {
            ...
            $html .= '<input id="' . esc_attr( $args['field_name'] ) . '" name="' . esc_attr( $args['field_name'] ) . '" type="checkbox" value="' . esc_attr( $args['field_value'] ) . '"' . ( $args['field_req'] ? ' aria-required="true"' : '' ) . ' />'
                    . ' <label for="' . esc_attr( $args['field_name'] ) . '" class="' . esc_attr( $args['field_req'] ? 'required' : 'optional' ) . '">' . wp_kses( $args['field_title'],'autoparts_kses_content' ) . '</label>';
        } else {
        ...
        if ( 'default' != $args['form_style'] ) {
            $html .= '<span class="sc_form_field_hover">'
                        ...
                        . '<span class="sc_form_field_content" data-content="' . esc_attr( $args['field_title'] ) . '">' . wp_kses( $args['field_title'],'autoparts_kses_content' ) . '</span>'
                    . '</span>';
        }
        $html .= '</span></div>';
        return $html;
